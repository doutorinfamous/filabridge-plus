...
// GenerateBambuHAPackage generates a complete HA package YAML for one Bambu printer.
func GenerateBambuHAPackage(printer BambuPrinter, webhookURL string) string {
	printer.Prefix = normalizeBambuHAPrefix(printer.Prefix)
	allTrays := CollectBambuTrayInfos(printer)
	automations := generateBambuAutomationsYAML(printer.Prefix, allTrays, webhookURL, printer)
	config := generateBambuConfigurationYAML(printer.Prefix, allTrays, printer, webhookURL)
	return fmt.Sprintf("# FilaBridge HA package for printer: %s\n\n%s\n\n%s", printer.Name, config, automations)
}

func buildTrayEntityLookup(allTrays []BambuTrayInfo) string {
	var parts []string
...
func uniqueAMSNumbers(trays []BambuTrayInfo) []int {
	seen := make(map[int]bool)
	var nums []int
	for _, t := range trays {
		if !seen[t.AMSNumber] {
			seen[t.AMSNumber] = true
			nums = append(nums, t.AMSNumber)
		}
	}
	return nums
}
...
func generateBambuAutomationsYAML(prefix string, allTrays []BambuTrayInfo, webhookURL string, printer BambuPrinter) string {
