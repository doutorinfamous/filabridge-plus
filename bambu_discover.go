...
func getEffectiveTranslationKey(entity HAEntityRegistryEntry) string {
...
func pickBestHAEntity(candidates []HAEntityRegistryEntry, stateMap map[string]HAState) *HAEntityRegistryEntry {
	if len(candidates) == 0 {
		return nil
	}
	if len(candidates) == 1 {
...
func parseAmsNumber(device HADeviceRegistryEntry) int {
...
func formatBambuTrayDisplayName(printerName string, amsNumber, trayNumber int, isExternal bool) string {
	if isExternal {
...
func resolveBambuJobName(printStatus HAState) string {
...
func parseHAStartTimeElapsed(startState HAState) float64 {
...
func enrichBambuPrintJob(printer *BambuPrinter, stateMap map[string]HAState, printStatus HAState, findEntity func(string) string) {
	printer.State = normalizeBambuState(printStatus.State)
	if printer.State != StatePrinting {
		return
	}
...
// bambuPrinterToPrinterData converts a BambuPrinter snapshot to PrinterData for WebSocket/status.
func bambuPrinterToPrinterData(printer BambuPrinter) PrinterData {
	data := PrinterData{
		Name:  printer.Name,
		State: printer.State,
	}
...
// DiscoverBambuPrinters discovers Bambu Lab printers from ha-bambulab via HA registries.
func DiscoverBambuPrinters(ha *HAClient) ([]BambuPrinter, error) {
	...
	stateMap := make(map[string]HAState, len(states))
	for _, s := range states {
		stateMap[s.EntityID] = s
	}

	deviceEntityMap := make(map[string][]HAEntityRegistryEntry)
	var bambuEntities []HAEntityRegistryEntry
	...
	var printerEntities []HAEntityRegistryEntry
	...
	for _, printerEntity := range printerEntities {
		...
		var candidates []HAEntityRegistryEntry
		...
		var printerDevice *HADeviceRegistryEntry
		for i := range devices {
		...
		var amsUnits []BambuAMS
		var externalSpools []BambuTray
		...
		for _, childDevice := range devices {
			...
			var trayEntities []HAEntityRegistryEntry
			var extEntities []HAEntityRegistryEntry
			...
			if len(trayEntities) > 0 {
				...
				for _, trayEntity := range trayEntities {
					...
					var sameTray []HAEntityRegistryEntry
					...
					ams.Trays = append(ams.Trays, BambuTray{
						...
						DisplayName:     formatBambuTrayDisplayName(name, amsNumber, trayNum, false),
					})
				}
			...
			if len(extEntities) > 0 {
				...
				if bestExt != nil {
					...
					externalSpools = append(externalSpools, BambuTray{
						...
						DisplayName:     formatBambuTrayDisplayName(name, 0, 0, true),
					})
				}
			}
		}
...
// CollectBambuTrayInfos collects tray info for HA config generation.
func CollectBambuTrayInfos(printer BambuPrinter) []BambuTrayInfo {
	var trays []BambuTrayInfo
	for i, ext := range printer.ExternalSpools {
		trays = append(trays, BambuTrayInfo{
