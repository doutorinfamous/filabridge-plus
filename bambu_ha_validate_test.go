...
func TestValidateFilaBridgeHAEntitiesMeterMissing(t *testing.T) {
	states := []HAState{
		{EntityID: "sensor.filabridge_03919c461204338_filament_usage", State: "1.56"},
	}
	result := ValidateFilaBridgeHAEntities("03919c461204338", states)
	if result.AllOK {
...
func TestValidateFilaBridgeHAEntitiesAllPresent(t *testing.T) {
	...
	result := ValidateFilaBridgeHAEntities(prefix, states)
	if !result.AllOK {
